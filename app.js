...
async function saveCompaniesToDatabase(companies) {
  ...
  if (!config.apiUrl) {
    console.warn('API URL not configured, skipping database save');
    return;
  }
...
async function loadCompaniesFromDatabase() {
  ...
  if (!config.apiUrl) {
    console.warn('API URL not configured, skipping database load');
    return;
  }
  ...
  try {
    ...
    hideLoading();
    console.error('[DB] Failed to load companies:', error);
    // Don't show error toast - user can search manually
  }
}
...
async function handleRingSearch(ringId) {
  ...
  const ringButton = document.querySelector(`.tool-item[data-ring="${ringId}"]`);

  // Set button to loading state
  if (ringButton) {
    ringButton.classList.add('tool-item--loading');
    const rightSpan = ringButton.querySelector('.tool-item__right');
    if (rightSpan) {
      rightSpan.innerHTML = '<span class="tool-item__spinner"></span>';
    }
  }
  ...
  const restoreButtonState = () => {
    if (ringButton) {
      ringButton.classList.remove('tool-item--loading');
      const rightSpan = ringButton.querySelector('.tool-item__right');
      if (rightSpan) {
        rightSpan.innerHTML = `
          <svg viewBox="0 0 20 20" aria-hidden="true">
            <circle cx="10" cy="10" r="8"></circle>
            <path d="M9 7l4 3-4 3z"></path>
          </svg>
        `;
      }
    }
  ...
  try {
    ...
    await saveCompaniesToDatabase(processedCompanies);
...
function handleFileSelect(file) {
  ...
  if (!validateFileType(file)) {
    showToast('Неподдерживаемый формат файла. Используйте Excel (.xlsx, .xls) или CSV (.csv)', 'error');
    return;
  }
  ...
  if (selectedFileInfo) selectedFileInfo.classList.remove('is-hidden');
...
function removeSelectedFile() {
  ...
  if (selectedFileInfo) selectedFileInfo.classList.add('is-hidden');
...
async function handleFileUpload() {
  ...
  try {
    ...
    const response = await fetch(`${config.apiUrl}/upload-pricelist`, {
      method: 'POST',
      body: formData
    });
    ...
    hideLoading();
    showToast('Прайс-лист успешно загружен', 'success');
    removeSelectedFile();
  ...
  if (dropZone) {
    ...
    ['dragenter', 'dragover'].forEach(eventName => {
      dropZone.addEventListener(eventName, () => {
        dropZone.classList.add('file-drop-zone--dragover');
      });
    });

    ['dragleave', 'drop'].forEach(eventName => {
      dropZone.addEventListener(eventName, () => {
        dropZone.classList.remove('file-drop-zone--dragover');
      });
    });
